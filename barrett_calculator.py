...
class BarrettCalculator:
    ...
    def save_patient_data(self, df: pd.DataFrame) -> None:
        """更新された患者データをExcelファイルに保存"""
        try:
            # バックアップファイルを作成
            backup_path = self.excel_file_path.with_suffix('.backup.xlsx')
            if self.excel_file_path.exists():
                self.excel_file_path.rename(backup_path)
                self.logger.info(f"バックアップファイルを作成: {backup_path}")
            
            # 更新されたデータを保存
            df.to_excel(self.excel_file_path, index=False)
            self.logger.info(f"データを保存しました: {self.excel_file_path}")
    ...
    def calculate_and_get_result(self, page, target_iol_power: float) -> Optional[float]:
        ...
        try:
            ...
            universal_formula_btn = page.locator('a:has-text("Universal Formula"), button:has-text("Universal Formula")').first
    ...
    def process_all_patients(self) -> None:
        ...
        try:
            ...
            print(f"結果ファイル: {self.excel_file_path}")
